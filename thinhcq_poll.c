	...
	while (1) {
		...
		if (fds[0].revents & POLLIN) {
			...
			if (res0 > 0) {
				...
				if(tmp0 != 0){
			            printf("flag_Ctrl has value 1 but flag_A = %d\n",flag_A);
				    flag_Ctrl =1;
				} else {
					printf("flag_Ctrl has value 0 but flag_A = %d\n",flag_A);
				    flag_Ctrl = 0;
				}
			}
		...
		if (fds[1].revents & POLLIN) {
                       ...
                       if (res1 > 0) {
                               tmp1 = *((uint32_t *)(gpios1 + GPIO_DATA_OFFSET / sizeof(uint32_t)));
			       if(tmp1 != 0){
				       printf("finally flagA = 1 but flag_Ctrl = %d\n",flag_Ctrl);
				       flag_A = 1;
			       } else {
				       printf("finally flagA = 0 but flag_Ctrl = %d\n",flag_Ctrl);
				        flag_A = 0;
		...
		if(flag_Ctrl == 1){
			printf("flag_Ctrl = 1\n");
			printf("flag A value in flag_Ctrl = %d\n",flag_A);
			if(flag_A == 1){
			   *((uint32_t *)(gpios0 + GPIO2_DATA_OFFSET / sizeof(uint32_t))) = 1;
			   printf("CTRL A has been press \n");
			} else {
			   *((uint32_t *)(gpios0 + GPIO2_DATA_OFFSET / sizeof(uint32_t))) = 0;
			}
